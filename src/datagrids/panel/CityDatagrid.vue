    ...
    <div class="panel-city-datagrid">
        <vue-good-table :columns="columns" :rows="rows" :search-options="options.search" :select-options="options.select"
            ...
            <template #table-row="props">
                ...
                <div v-else-if="props.column.field == 'province_id'">
                    {{ props.row.province.title }}
                </div>
...
import { delete_city, get_data } from '@/services/city.service'
import { AskPrompt, Toast } from '@/helpers/Base';

export default defineComponent({
    name: 'panel-city-datagrid',
    components: {
        VueGoodTable,
        Button,
        AkPlus
        ...
        deleteAction() {
            ...
            AskPrompt('آیا از انجام اینکار مطمئن هستید؟', 'warning').then(async result => {
                if (result.isConfirmed) {
                    const ids = (this.$refs['panel-city-datagrid'] as any).selectedRows.map((i: any, index: number) => {
                    ...
                    const result = await delete_city(ids)
    ...
    async mounted() {
        ...
        if (res.status == 200)
            this.rows = res.data.row.city_list
    }
})
</script>
