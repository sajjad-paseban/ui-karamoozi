<template>
    <div class="panel-degree-datagrid">
...
import { delete_degree, get_data } from '@/services/degree.service'
import { AskPrompt, Toast } from '@/helpers/Base';

export default defineComponent({
    name: 'panel-degree-datagrid',
    components: {
        VueGoodTable,
        Button,
        AkPlus
        ...
        deleteAction() {
            ...
            AskPrompt('آیا از انجام اینکار مطمئن هستید؟', 'warning').then(async result => {
                if (result.isConfirmed) {
                    const ids = (this.$refs['panel-degree-datagrid'] as any).selectedRows.map((i: any, index: number) => {
                    ...
                    const result = await delete_degree(ids)
    ...
    async mounted() {
        ...
        if (res.status == 200)
            this.rows = res.data.row.degree_list
    }
})
</script>

<style lang="scss" scoped>
.panel-degree-datagrid {
    p.empty-state {
        text-align: center;
    }
}
