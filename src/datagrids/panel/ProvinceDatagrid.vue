<template>
    <div class="panel-province-datagrid">
...
import { delete_province, get_data } from '@/services/province.service'
import { AskPrompt, Toast } from '@/helpers/Base';

export default defineComponent({
    name: 'panel-province-datagrid',
    components: {
        VueGoodTable,
        Button,
        AkPlus
        ...
        deleteAction() {
            ...
            AskPrompt('آیا از انجام اینکار مطمئن هستید؟', 'warning').then(async result => {
                if (result.isConfirmed) {
                    const ids = (this.$refs['panel-province-datagrid'] as any).selectedRows.map((i: any, index: number) => {
                    ...
                    const result = await delete_province(ids)
    ...
    async mounted() {
        ...
        if (res.status == 200)
            this.rows = res.data.row.province_list
    }
})
</script>

<style lang="scss" scoped>
.panel-province-datagrid {
    p.empty-state {
        text-align: center;
    }
}
