...
import { delete_role, get_data } from '@/services/role.service'
import { AskPrompt, Toast } from '@/helpers/Base';

export default defineComponent({
    name: 'panel-role-datagrid',
    components: {
        VueGoodTable,
        Button,
        AkPlus
        ...
        deleteAction() {
            ...
            AskPrompt('آیا از انجام اینکار مطمئن هستید؟', 'warning').then(async result => {
                if (result.isConfirmed) {
                    const ids = (this.$refs['panel-role-datagrid'] as any).selectedRows.map((i: any, index: number) => {
                    ...
                    const result = await delete_role(ids)
    ...
    async mounted() {
        ...
        if (res.status == 200)
            this.rows = res.data.row.role_list
    }
})
</script>
