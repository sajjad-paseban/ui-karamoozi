...
import { find_data, update_city } from '@/services/city.service'
...
export default defineComponent({
    ...
    methods: {
        ...
        handleSubmit: async function (values: any, { resetForm }: any) {
            ...
            const res = await update_city(values, this.props.id)
    ...
    async mounted() {
        ...
        if (result.status == 200) {
            this.form.params.title = result.data.row.city_list.title
            this.form.params.province_id = result.data.row.city_list.province_id
            this.form.params.status = result.data.row.city_list.status == 1 ? true : false
        }
