...
import { find_data, update_color } from '@/services/color.service'
export default defineComponent({
    ...
    methods: {
        ...
        handleSubmit: async function (values: any, { resetForm }: any) {
            ...
            const res = await update_color(values, this.props.id)
    ...
    async mounted() {
        ...
        if (result.status == 200) {
            this.form.params.title = result.data.row.color_list.title
            this.form.params.color = result.data.row.color_list.color
            this.form.params.status = result.data.row.color_list.status == 1 ? true : false
        }
    }
})
</script>
