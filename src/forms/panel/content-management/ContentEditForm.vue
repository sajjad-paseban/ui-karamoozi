...
import { ContentEditFormSchema } from '@/forms/Schema'
import { Toast } from '@/helpers/Base'
import { update_content, find_data } from '@/services/content.service'
...
export default defineComponent({
    ...
    methods: {
        handleSubmit: async function (values: any, { resetForm }: any) {
            ...
            const res = await update_content(values, this.route.params.id)
        ...
        hasStarHandler: function (el: any) {
    ...
    async mounted() {
        const res = await find_data(this.route.params.id)
        if (res.status == 200) {
            const data = res.data.row.content_list
            this.form.params.title = data.title
            this.form.params.seo_description = data.seo_description
            this.form.params.content = data.content
            this.form.params.status = data.status == 1 ? true : false
            this.form.params.has_star = data.has_star == 1 ? true : false
        }
...
<template>
    <Form :validation-schema="form.ContentEditFormSchema" @submit="handleSubmit">
        ...
        <div class="d-flex">
            <div class="form-group w-100 m-1">
                ...
                <div class="form-check form-switch d-flex flex-column align-items-end">
                    <input type="checkbox" :checked="form.params.has_star" @change="hasStarHandler" name="has_star"
                        id="has_star" class="form-check-input">
                    <Field hidden type="checkbox" @change="hasStarHandler" name="has_star" id="has_star"
                        class="form-check-input" />
