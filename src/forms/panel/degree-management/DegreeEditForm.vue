<template>
    <Form :validation-schema="form.DegreeFormSchema" @submit="handleSubmit">
...
import { DegreeFormSchema } from '@/forms/Schema'
import { handleNumber, Toast } from '@/helpers/Base'
import { find_data, update_degree } from '@/services/degree.service'
export default defineComponent({
    name: 'degree-edit-form',
    components: {
        Form,
        Field,
        ErrorMessage
    ...
    methods: {
        ...
        handleSubmit: async function (values: any, { resetForm }: any) {
            ...
            const res = await update_degree(values, this.props.id)
    ...
    async mounted() {
        ...
        if (result.status == 200) {
            this.form.params.title = result.data.row.degree_list.title
            this.form.params.status = result.data.row.degree_list.status == 1 ? true : false
        }
