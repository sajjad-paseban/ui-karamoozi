<template>
    <Form :validation-schema="form.ProvinceFormSchema" @submit="handleSubmit">
...
import { ProvinceFormSchema } from '@/forms/Schema'
import { handleNumber, Toast } from '@/helpers/Base'
import { find_data, update_province } from '@/services/province.service'
export default defineComponent({
    name: 'province-edit-form',
    components: {
        Form,
        Field,
        ErrorMessage
    ...
    methods: {
        ...
        handleSubmit: async function (values: any, { resetForm }: any) {
            ...
            const res = await update_province(values, this.props.id)
    ...
    async mounted() {
        ...
        if (result.status == 200) {
            this.form.params.title = result.data.row.province_list.title
            this.form.params.status = result.data.row.province_list.status == 1 ? true : false
        }
    }
})
</script>
