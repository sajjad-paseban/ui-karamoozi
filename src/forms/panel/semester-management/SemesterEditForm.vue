...
import { find_data, update_semester } from '@/services/semester.service'
...
export default defineComponent({
    ...
    methods: {
        ...
        handleSubmit: async function (values: any, { resetForm }: any) {
            ...
            const res = await update_semester(values, this.route.params.id as unknown as number)
    ...
    async mounted() {
        ...
        if (result.status == 200) {
            this.form.params.name = result.data.row.semester_list.name
            this.form.params.code = result.data.row.semester_list.code
            this.form.params.is_active = result.data.row.semester_list.is_active == 1 ? true : false
        }
