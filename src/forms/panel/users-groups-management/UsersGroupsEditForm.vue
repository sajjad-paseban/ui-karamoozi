...
import { update_user_group, find_data, get_users_by_role } from '@/services/users_groups.service'
...
export default defineComponent({
    ...
    methods: {
        ...
        handleSubmit: async function (values: any, { resetForm }: any) {
            ...
            const res = await update_user_group(values, this.route.params.id as unknown as number)
    ...
    async mounted() {
        ...
        if (result.status == 200) {
            this.form.params.role_id = result.data.row.users_groups_list.role_id
            this.form.params.userid = result.data.row.users_groups_list.user.id
            this.form.params.group_id = result.data.row.users_groups_list.group.id
            this.form.params.status = result.data.row.users_groups_list.status == 1 ? true : false
        }

        const user_res = await get_users_by_role(this.form.params.role_id as unknown as number)
        if (user_res.status == 200)
            this.users = user_res.data.row.users
